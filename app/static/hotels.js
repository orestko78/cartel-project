...
const translations = {
    uk: {
        ...
        hero_title: "Мережа готелів <span>CARTEL</span>", 
        hero_desc: "Затишні номери, неперевершені краєвиди Карпат та високий рівень сервісу для вашого ідеального відпочинку.",
        
        card_hotel: "🏨 Готель",
        btn_visit: "Резерв номера"
    },
    en: {
        nav_about: "About Us", 
        ...
        hero_title: "Hotels Network <span>CARTEL</span>", 
        hero_desc: "Cozy rooms, stunning Carpathian views, and a high level of service for your perfect vacation.",
        
        card_hotel: "🏨 Hotel",
        btn_visit: "Book a room"
    }
};
...
async function loadEstablishments() {
    try {
        ...
        // Фільтруємо саме готелі
        const hotels = data.filter(item => {
            if (!item.type) return false;
            const t = item.type.toLowerCase();
            return t === 'hotel' || t === 'готель';
        });

        if (hotels.length === 0) {
            grid.innerHTML = '<p class="empty-state">Наразі немає доданих готелів.</p>';
            return;
        }
